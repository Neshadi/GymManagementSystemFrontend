...
const BookDemo = () => {
  const BASE_URL = "https://gymmanagementsystembackend-1.onrender.com";
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/demo/book`, user, { headers });
  ...
  const handleUpdate = async (query) => {
    ...
    try {
      // Correcting the axios.get syntax by including params within the options object
      const response = await axios.get(`${BASE_URL}/demo/findBookByQuery`, {
        headers: headers,
        params: { query: query }
      });
  ...
  const handleUpdate1 = async (id) => {
    ...
    if (editing && id) {
      ...
      try {
        ...
        const response = await axios.put(`${BASE_URL}/demo/updateBook/${id}`, updatedDemo, { headers });
  ...
  const handleDelete1 = async (id) => {
    ...
    // Check if a field is selected for deletion
    if (!deleting) {
    ...
    if (deleting && id) {
      try {
        const response = await axios.delete(`${BASE_URL}/demo/delete/${id}`, { headers });

        if (response.status === 200) {
          setSearchResults(false);  // Clear search results if needed
  ...
  const handleDemo = async () => {
    ...
    try {
      const response = await axios.get(`${BASE_URL}/demo/findBook`, { headers });
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ textAlign: 'center' }}>
          <button
            type="submit"
            style={{
              backgroundColor: isHovered ? 'brown' : 'blue',
              color: 'white',
              width: '105%',
              padding: '10px',
              border: 'none',
